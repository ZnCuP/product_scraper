...
TARGET_KEYWORDS = [
    ...
    "MAP Sensor",
    "Exhaust Gas PDF Differential Pressure Sensor",
    "EGTS Sensor",
    ...
    "Fuel Injection Pressure Control Regulator",
...
REQUEST_DELAY_SEC = (
    2.5  # 每次成功页面请求后的固定延迟（秒），用于模拟人类行为和避免过快请求
)

# Playwright浏览器配置
PLAYWRIGHT_TIMEOUT_MS = 90000  # Playwright操作的默认超时时间（毫秒）
PLAYWRIGHT_HEADLESS = (
    True  # 是否无头模式运行浏览器 (True: 后台运行, False: 显示浏览器窗口)
)
